...
import { useGetCategories } from "./hooksAddEditBook";
import { useGetDetailBook } from "../DetailBook/hooksDetailBook";
import EditBookAdminFormSkeleton from "./EditBookAdminFormSkeleton";
...
const EditBookAdmin = () => {
    ...
    const isPendingEditBook = false;

    useEffect(() => {
        if (dataDetailBook) {
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setTitle(dataDetailBook.title);
            setAuthor(dataDetailBook.author.name);
            setCategory(dataDetailBook.categoryId);
            setNumberOfPage(dataDetailBook.pages);
            setDescription(dataDetailBook.description);
            setPreviewUrl(dataDetailBook.coverImage);
        }
    }, [dataDetailBook]);
            ...
            <main className=" min-h-screen pt-23 md:px-0 px-4 w-full md:max-w-300 mx-auto gap-12 grid mb-5">
                <div className="flex flex-col w-full max-w-132.25 gap-8 mx-auto">
                    ...
                    <a href="/adminpage#booklist" className="flex gap-3 text-sm font-semibold items-center">
                        ...
                        <span className="text-xl md:text-display-xs">Edit Book</span>
                    </a>


                    {
                        isLoadingDetailBook && (<EditBookAdminFormSkeleton />)
                    }

                    {
                        !isLoadingDetailBook && (
                            ...
                            <form method="POST" onDragOver={handleDragOver}
                                ...
                                <div className="grid gap-2">
                                    <Label htmlFor="bio" className="text-sm font-bold">Category {category}</Label>
                                    <Field data-invalid={!authorValid} className="relative">
                                        <select
                                            disabled={isPendingEditBook}
                                            id="authorname"
                                            name="authorname"
                                            className="py-2 pl-4 pr-10 border rounded-xl appearance-none bg-no-repeat bg-right" 
                                            required
                                            value={category}
                                        ...
                                        {!authorValid && (<FieldLabel className="text-xs text-accent-red">Category required</FieldLabel>)}
                                    </Field>
                                </div>
                                ...
                                <Button
                                    disabled={isPendingEditBook}
                                    type="submit"
                                    className="w-full rounded-full h-12 text-sm">{isPendingEditBook && (<Spinner />)} Save</Button>
